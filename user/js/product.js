document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!window.location.pathname.includes('product_page.html')) return;

    // Получаем ID товара из URL (например: product_page.html?id=1)
    const urlParams = new URLSearchParams(window.location.search);
    const productId = urlParams.get('id');

    // База данных товаров (замените на свои данные)
    const products = {
        '1': {
    ...
    if (productId && products[productId]) {
        ...
        document.title = `${product.title} | ИскраСвет`; // Обновляем title страницы
    } else {
